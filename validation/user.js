...
const userValidation = {
  validateUsername: () =>
    body("username")
      ...
      .isLength({ max: 24 })
      .withMessage("username too long. 24 characters max"),
  ...
  validateNameField: (nameField) =>
    body(nameField)
      ...
      .notEmpty()
      .escape()
      .withMessage(`Invalid name field: ${nameField}`),
  ...
  validatePassword: () =>
    body("password")
      ...
      .withMessage(
        "password must be greater than 6 characters, and less than 256"
      ),
  ...
  validateRole: () =>
    body("role").isIn(User.VALID_ROLES).withMessage("invalid Role"),

  validateOptionalSessions: () =>
    body("sessions.*.weekNumber")
      .optional()
      .isInt()
      .withMessage("Session weekNumber must be an int"),
  ...
  validateUpdateUsernameUnique: () =>
    body("username").custom(async (bodyUsername, { req }) => {
  ...
  validateUserSessionList: (fieldName) =>
    body(fieldName)
      .exists()
      .isInt()
      .withMessage("not int")
      .custom(async (sid, { req }) => {
        //validate staffSessionIds and add to request array
        const staffSession = await StaffSession.get(sid);
        if (!staffSession) {
          throw new Error(`Staff session with id ${sid} not found`);
        }
